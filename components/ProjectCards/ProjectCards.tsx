    ...
    <>
      {projects.map((project, index) => (
        ...
        >
          ...
          >
            <button
              className="study_case relative flex w-full items-center justify-between overflow-hidden rounded-full px-3 py-2 text-sm "
              onClick={() => router.push(`/projects/${project.id}`)}
              onMouseEnter={() => setJustHoverIndex(index)}
              onMouseLeave={() => setJustHoverIndex(-1)}
            >
              <span className="relative z-10">View Project Detail</span>
